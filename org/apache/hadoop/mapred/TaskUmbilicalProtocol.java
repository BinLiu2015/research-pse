...
public interface TaskUmbilicalProtocol extends VersionedProtocol {
   ...
   * 
   * Version 20 Added isSpeculative(): let Child know whether the launching task is speculative or not
   *            Modified returned value {boolean} of statusUpdate() and ping() to TrackerChildResponse
   *            Added getTrackerAddress(): let child know where the task is launched.
   * */
   ...
   */
  TrackerChildResponse statusUpdate(TaskAttemptID taskId, TaskStatus taskStatus) 
   ...
   * @return True if the task is known
   */
  TrackerChildResponse ping(TaskAttemptID taskid) throws IOException;
  ...
  /**
   * Author ygwang
   * child want to know where the task is launched, for original task publishing its checkpoint data
   * @return
   */
